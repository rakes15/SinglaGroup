...
public class DatabaseSqlLiteHandlerFilter{
    ...
    public void insertFilterTable(List<Map<String,String>> list) {
        ...
        try {
            ...
            for(int x=0; x<list.size(); x++){
                ...
                String TotalItem = (list.get(x).get("TotalItem")==null || list.get(x).get("TotalItem").isEmpty())?"":(list.get(x).get("TotalItem").equals("0")?"":"("+list.get(x).get("TotalItem")+")");
    ...
    public String[] ApplyFilter(int StrLength) {
        ...
        if(cursor.getCount()>0)
        {
            cursor.moveToFirst();
            do {
                int Seq = cursor.getInt(cursor.getColumnIndex(SINGLA_FILTER_SEQUENCE));
                str[Seq]=(str[Seq]==null)?"":str[Seq];
                str[Seq] += "'"+cursor.getString(cursor.getColumnIndex(SINGLA_FILTER_ATTR_ID))+"',";
            } while (cursor.moveToNext());
            // closing connection
            cursor.close();
            db.close();
    ...
    public String[] ApplyFilterInventory(int StrLength) {
        String[] str=new String[StrLength];
        // Select All Query
        String selectQuery = "SELECT DISTINCT "+SINGLA_FILTER_ATTR_ID+","+SINGLA_FILTER_ATTR_NAME+","+SINGLA_FILTER_SEQUENCE+" from " + SINGLA_FILTER_TABLE+" WHERE "+SINGLA_FILTER_FLAG+"=1";
        //System.out.println(selectQuery);
        DatabaseSqliteRootHandler rootHandler = new DatabaseSqliteRootHandler(context);
        SQLiteDatabase db = rootHandler.getReadableDatabase();
        Cursor cursor = db.rawQuery(selectQuery,null);
        ...
        if(cursor.getCount()>0)
        {
            cursor.moveToFirst();
            do {
                int Seq = cursor.getInt(cursor.getColumnIndex(SINGLA_FILTER_SEQUENCE))-1;
                str[Seq]=(str[Seq]==null)?"":str[Seq];
                str[Seq] += (Seq == 10 || Seq == 13 ? cursor.getString(cursor.getColumnIndex(SINGLA_FILTER_ATTR_NAME))+"," : "'"+cursor.getString(cursor.getColumnIndex(SINGLA_FILTER_ATTR_ID))+"',");
            } while (cursor.moveToNext());
            // closing connection
            cursor.close();
            db.close();
    ...
    public List<Map<String,String>> CaptionWithSeqInventory(){
        List<Map<String,String>> dataList=new ArrayList<Map<String,String>>();
        // Select All Query
        String selectQuery = "SELECT DISTINCT "+SINGLA_FILTER_SEQUENCE+" from " + SINGLA_FILTER_TABLE+" ORDER BY "+SINGLA_FILTER_SEQUENCE+" ASC" ;
        //System.out.println(selectQuery);
        ...
        if(cursor.getCount()>0)
        {
            ...
            do {
                int Seq = (cursor.getInt(cursor.getColumnIndex(SINGLA_FILTER_SEQUENCE))-1);
                if(StaticValues.FilterCaptionSequenceInventory[Seq]!=null) {
                    Map<String, String> map = new HashMap<String, String>();
                    map.put("Seq", cursor.getString(cursor.getColumnIndex(SINGLA_FILTER_SEQUENCE)));
                    map.put("Caption", StaticValues.FilterCaptionSequenceInventory[Seq]);
                    dataList.add(map);
                }
                System.out.println("Seq:"+Seq+"\t Seq1:"+cursor.getInt(cursor.getColumnIndex(SINGLA_FILTER_SEQUENCE)));
            } while (cursor.moveToNext());
            // closing connection
            cursor.close();
            db.close();
