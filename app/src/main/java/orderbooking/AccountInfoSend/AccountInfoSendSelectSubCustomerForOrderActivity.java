...
import orderbooking.AccountInfoSend.adapter.AccountInfoSendSelectSubCustomerForOrderAdapter;
import orderbooking.StaticValues;
import orderbooking.customerlist.adapter.SelectSubCustomerForOrderAdapter;
...
public class AccountInfoSendSelectSubCustomerForOrderActivity extends AppCompatActivity {
    ActionBar actionBar;
    ...
    private AccountInfoSendSelectSubCustomerForOrderAdapter adapter;
    ...
    private static String TAG = AccountInfoSendSelectSubCustomerForOrderActivity.class.getSimpleName();
    @Override
    ...
    private void Initialization(){
        this.context = AccountInfoSendSelectSubCustomerForOrderActivity.this;
    ...
    private void ModulePermission(){
        try {
            ...
            if (StaticValues.viewFlag == 1 || StaticValues.editFlag == 1 || StaticValues.createFlag == 1 || StaticValues.removeFlag == 1 || StaticValues.printFlag == 1 || StaticValues.importFlag == 1 || StaticValues.exportFlag == 1){
                ...
                MessageDialog.MessageDialog(AccountInfoSendSelectSubCustomerForOrderActivity.this,"Alert","You don't have permission of "+Title+" module");
            }
        }catch (Exception e){
            MessageDialog.MessageDialog(AccountInfoSendSelectSubCustomerForOrderActivity.this,"Exception",e.toString());
        }
    }
    private void CallApiMethod(){
        ...
        if (!status.contentEquals("No Internet Connection")) {
            ...
            String[] str = obj.GetSharePreferenceSession(AccountInfoSendSelectSubCustomerForOrderActivity.this);
            if (str!=null) {
                CallVolleySelectSubCustomerForOrder(str[2], str[3], str[4], str[0], str[16], str[5],str[14],PartyID);
            }
        } else {
            MessageDialog messageDialog=new MessageDialog();
            messageDialog.MessageDialog(AccountInfoSendSelectSubCustomerForOrderActivity.this,"","","No Internet Connection");
        }
        ...
        NetworkChangeReceiver.bindNetworkListener(new NetworkListener() {
            ...
            public void networkReceived(String status) {
                if (!status.contentEquals("No Internet Connection")) {
                    ...
                    MessageDialog messageDialog=new MessageDialog();
                    messageDialog.MessageDialog(AccountInfoSendSelectSubCustomerForOrderActivity.this,"","",status);
                }
            }
        });
    }
            ...
            public void onResponse(String response) {
                ...
                try{
                    ...
                    if (Status == 1) {
                        ...
                        adapter=new AccountInfoSendSelectSubCustomerForOrderAdapter(context,list);
                        ...
                        ItemClickSupport.addTo(recyclerView).setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
                            ...
                            public void onItemClicked(RecyclerView recyclerView, int position, View v) {
                                    SelectSubCustomerForOrderDataset dataset = (SelectSubCustomerForOrderDataset) adapter.getItem(position);
...
//                System.out.println("on query submit: "+query);
    ...
    //TODO: Dialog share options
    private void DialogShareOptions(final SelectSubCustomerForOrderDataset dataset){
        final Dialog dialog = new Dialog(new ContextThemeWrapper(context, R.style.DialogSlideAnim));
    ...
    private void EmailSMSWhatsAppDialogFunction(final Context context, final SelectSubCustomerForOrderDataset dataset, final int flag){
                ...
                if (flag == 0){
                    ...
                    if (cancel) {
                        ...
                        if (Mobile.length() == 10){
                            ...
                            if (!status.contentEquals("No Internet Connection")) {
                                CallVolleySend(Mobile,"","0",dataset.getAccountNo(),dataset.getIFSCCOde(),dataset.getAccountHolderName(),dataset.getSubPartyName());
                                dialog.dismiss();
                            }else{
                    ...
                    if (cancel) {
                        ...
                        if (ValidationMethods.isEmailValid(Email)){
                            String status = NetworkUtils.getConnectivityStatusString(context);
                            if (!status.contentEquals("No Internet Connection")) {
                                CallVolleySend("",Email,"1",dataset.getAccountNo(),dataset.getIFSCCOde(),dataset.getAccountHolderName(),dataset.getSubPartyName());
                                dialog.dismiss();
                            }else{
                                MessageDialog.MessageDialog(context,"","No Internet Connection");
                            }
                    ...
                    if (cancel) {
                        ...
                        if (Mobile.length() == 10 && ValidationMethods.isEmailValid(Email)){
                            String status = NetworkUtils.getConnectivityStatusString(context);
                            if (!status.contentEquals("No Internet Connection")) {
                                CallVolleySend(Mobile,Email,"2",dataset.getAccountNo(),dataset.getIFSCCOde(),dataset.getAccountHolderName(),dataset.getSubPartyName());
                                dialog.dismiss();
                            }else{
                                MessageDialog.MessageDialog(context,"","No Internet Connection");
                            }
    ...
    private void setTableLayoutAccountInfo(SelectSubCustomerForOrderDataset dataset, TableLayout tableLayout){
