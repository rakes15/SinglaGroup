...
import whatsapp.DirectlyWhatsAppActivity;
...
public class BookOrderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	private int SingleOrMultiFlag;
	int count=0;
	boolean[] checked;
	public static List<CloseOrBookDataset> listMultiCustomer = new ArrayList<CloseOrBookDataset>();
	public BookOrderAdapter(@NonNull List<ListItem> items, Context context, int SingleOrMultiFlag) {
		this.items = items;
		this.context = context;
		this.filterItems = new ArrayList<ListItem>();
		this.filterItems.addAll(this.items);
		this.SingleOrMultiFlag = SingleOrMultiFlag;
		checked= new boolean[filterItems.size()];
	...
	public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
		...
		switch (viewType) {
			...
			case ListItem.TYPE_EVENT: {
				...
				if (SingleOrMultiFlag == 0){
					holder.checkBox.setVisibility(View.GONE);
					holder.cardView.setTag(event.getEvent());
					holder.cardView.setOnClickListener(new View.OnClickListener() {
						@Override
						public void onClick(View view) {
							listMultiCustomer = new ArrayList<CloseOrBookDataset>();
							CloseOrBookDataset Dataset = (CloseOrBookDataset) view.getTag();
							if (Dataset!=null) {
								listMultiCustomer.add(Dataset);
								StaticValues.MultiOrderSize = listMultiCustomer.size();
					...
					holder.checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
						...
						public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
							...
							count=0;
							listMultiCustomer = new ArrayList<CloseOrBookDataset>();
							for(int z=0;z<filterItems.size();z++) {
								if(checked[z]==true) {
									count++;
									if(count>(filterItems.size()-1)) {
										checked[position]=false;
										holder.checkBox.setChecked(false);
										MessageDialog.MessageDialog(context, "","Book Order Maximum "+(filterItems.size()-1)+" Customers");
										notifyDataSetChanged();
										break;
									} else {
										if(filterItems.get(z).getType()==1){
											EventItem eventItem = (EventItem)filterItems.get(z);
											System.out.println("Party:"+eventItem.getEvent().getPartyName());
											listMultiCustomer.add(eventItem.getEvent());
										}
									}
								}
							}
							String msg = (listMultiCustomer.size()>0 ? ((listMultiCustomer.size()==1) ? "1 Party Selected" : listMultiCustomer.size()+ " Parties Seleted" ) : "" );
							if (!msg.isEmpty()) { Snackbar.make(holder.cardView,""+msg,Snackbar.LENGTH_SHORT).show(); }
						}
					});
				}
				break;
