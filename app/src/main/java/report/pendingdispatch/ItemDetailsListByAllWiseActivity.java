...
public class ItemDetailsListByAllWiseActivity extends AppCompatActivity {
        ...
        try {
            ...
            if(groupWise!=null) {
                ...
                actionBar.setTitle("" + groupWise.getGroupName() + " ( "+groupWise.getMainGroup() + " ) ");
                 if (!groupWise.getOrderID().isEmpty() && !groupWise.getGroupID().isEmpty()) {
                    LoadRecyclerView(groupWise.getGroupID(), "" ,groupWise.getOrderID(),0);
                }else if (!groupWise.getPartyID().isEmpty() && !groupWise.getGroupID().isEmpty()) {
                    LoadRecyclerViewPartyWise(groupWise.getGroupID(), PartyID, groupWise.getSubPartyID(),groupWise.getRefName());
    ...
    private void LoadRecyclerView(String GroupID, final String PartyID, final String OrderID, int flag){
        ...
        if (!OrderID.isEmpty() && OrderID!=null && !GroupID.isEmpty() && GroupID!=null) {
            adapter = new ItemDetailsAdapter(context,DBHandler.getItemListByOrderWise(GroupID,OrderID));
    ...
    private void LoadRecyclerViewPartyWise(String GroupID, final String PartyID, final String SubPartyID,String RefName){
        //TODO: Load Recycler View
        if (!PartyID.isEmpty() && PartyID!=null && !GroupID.isEmpty() && GroupID!=null) {
            adapter = new ItemDetailsAdapter(context,DBHandler.getItemListByPartyWise(GroupID,PartyID,SubPartyID,RefName));
            recyclerView.setAdapter(adapter);
            LinearLayoutManager linearLayoutManager = new LinearLayoutManager(context);
            linearLayoutManager.setOrientation(LinearLayoutManager.VERTICAL);
            recyclerView.setLayoutManager(linearLayoutManager);
